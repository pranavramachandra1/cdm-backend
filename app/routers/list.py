...
async def create_list(list_data: ListCreate, list_service: ListService = Depends(get_list_service)):
    """Create a new list"""
...
@handle_list_exceptions
async def update_list(list_id: str, list_data: ListUpdate, list_service: ListService = Depends(get_list_service)):
    """Update a list by ID"""
    return list_service.update_list(list_id, list_data)

@router.delete("/{list_id}", status_code=status.HTTP_200_OK)
@handle_list_exceptions
async def delete_list(list_id: str, list_service: ListService = Depends(get_list_service)):
    """Delete a list by ID"""
...
@router.get("/user/{user_id}", response_model=List[ListResponse])  # ✅ Better URL structure
@handle_list_exceptions
async def get_lists_by_user(user_id: str, list_service: ListService = Depends(get_list_service)):
...
@router.patch("/{list_id}/increment-version", response_model=ListResponse)  # ✅ Better URL structure
@handle_list_exceptions
async def increment_list_version(list_id: str, list_service: ListService = Depends(get_list_service)):
...
@handle_list_exceptions
async def get_list_statistics(list_id: str, list_service: ListService = Depends(get_list_service)):
    """
